			...
			<table class ="gold">
				...
				<c:forEach items="${InFo}" var="Info1">
					<tr>
						...
						<td><a href="javascript:openWindowPop('/system/detailPopup.do?bbsId=${Info1.bbsId}', '상세정보');">상세정보</a></td>
					</tr>
				</c:forEach>
			</table>	
		</body>

		<a href="javascript:del_all('chk[]');"><input type="button" value="일괄삭제"/></a>
		<a href="/system/mulInputPage.do"><input type="button" value="일괄작성"></a><br>
...
// 		int listCount = SELECT COUNT(*) FROM BOARD; //총 게시물 수를 불러온다.
// 		int PAGE_LIMIT = 5;  // 한번에 보이는 페이징 수
// 		int BOARD_LIMIT = 3; // 하나의 페이지에 보이는 게시물 수
	});
