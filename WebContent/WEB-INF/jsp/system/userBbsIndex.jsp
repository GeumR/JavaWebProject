...
<!-- 					<th>수정일자</th> -->
				...
				<c:forEach items="${InFo}" var="Info1">
					<tr>
<!-- 						<th><input type="checkbox" name='chk[]' value=${Info1.bbsId}> -->
...
<%-- 						<td><a href="javascript:openWindowPop('/system/detailPopup.do?bbsId=${Info1.bbsId}&bbsLevel=${Info1.bbsLevel}', '상세정보');">상세정보</a></td> --%>
			...
			<ul class="ulStluy1">
				<c:if test="${nowPage != 1}">
					<a class="box" href="/system/userBbsPage.do?nowPage=1"><<</a>
				</c:if>
				<c:if test="${nowPage > 1}">
					<a class="box" href="/system/userBbsPage.do?nowPage=${nowPage-1}"><</a>
				</c:if>
...
<!-- 				현재 페이지 표시 (현재페이지랑 번호가 같으면 파란색으로 표시)           -->
					...
					<c:if test="${nowPage != idx}">
						<a class="pageStluy1" href="/system/userBbsPage.do?nowPage=${idx}">${idx}</a>
					</c:if>
				</c:forEach>
				<c:if test="${TotalPage != endPage}">
					<a class="box" href="/system/userBbsPage.do?nowPage=${endPage+1}">></a>
				</c:if>
				<c:if test="${TotalPage != endPage}">
					<a class="box" href="/system/userBbsPage.do?nowPage=${TotalPage}">	>></a>
				</c:if>
			</ul>
